<script>
	...
	async function downloadPack() {
		Downloader.downloadPack($url.trim()).then(async (result) => {
			if (result) {
				step = 3;
	...
	{#if step === 1}
		<div class="form-control">
			<p style="font-size: 1rem; text-align: center; margin-bottom:0.65rem;">
				Get the download links from <a href="https://www.thekinemancer.com/" target="_blank">The Kinemancer's website</a
				>
			</p>
			<div>
