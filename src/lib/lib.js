...
export function getSceneDelegator() {
	...
	activeUsers.sort((a, b) => {
		return (
			(foundry.utils.getProperty(b, CONSTANTS.UPDATED_FLAG) ?? 0) -
			(foundry.utils.getProperty(a, CONSTANTS.UPDATED_FLAG) ?? 0)
		);
	});
		...
		if (state.start === CONSTANTS.START.PREV) {
			...
			if (previousState) {
				if (!isRealNumber(previousState.end)) {
					errors.push(
						`State "${state.name}" cannot have "prev" as its start time, because state "${previousState.name}" does not end at a specific time`
					);
				}
			} else {
		...
		if (state.end === CONSTANTS.END.NEXT) {
			...
			if (nextState) {
				if (!isRealNumber(nextState.start)) {
					errors.push(
						`State "${state.name}" cannot have "next" as its end time, because state "${nextState.name}" does not start at a specific time`
					);
				}
			} else {
...
export function determineFileColor(inFile) {
	...
	if (lowerCaseFile.includes("__")) {
		const fileColorName = lowerCaseFile.split("__")[1].split(".")[0].replace("_", " ");
