					...
					for (const zipFile of filesToCheck) {
						...
						if (zipFile.name.endsWith(".json")) {
							...
							try {
								...
								throw new Error(
									`Could not load JSON file for this pack! Please contact The Kinemancer for support | ${err}`
								);
							}
							let result = foundry.utils.mergeObject(jsonData, {});
							if (foundry.utils.getProperty(result, CONSTANTS.ASSET_TYPES_FLAG)?.length) {
								tags[GameSettings.SETTINGS.ASSET_TYPES][path] = foundry.utils.getProperty(
									result,
									CONSTANTS.ASSET_TYPES_FLAG
								);
							}
							if (foundry.utils.getProperty(result, CONSTANTS.TIME_PERIODS_FLAG)?.length) {
								tags[GameSettings.SETTINGS.TIME_PERIODS][path] = foundry.utils.getProperty(
									result,
									CONSTANTS.TIME_PERIODS_FLAG
								);
							}
							if (foundry.utils.getProperty(result, CONSTANTS.CATEGORIES_FLAG)?.length) {
								tags[GameSettings.SETTINGS.CATEGORIES][path] = foundry.utils.getProperty(
									result,
									CONSTANTS.CATEGORIES_FLAG
								);
							}
