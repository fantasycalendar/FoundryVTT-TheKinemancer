...
function dirMatchesFilterFn(filters, dir) {
	return Object.entries(filters).every(([settingsKey, tagFilters]) => {
		...
		return (
			setting[dir] &&
			Object.entries(tagFilters).every(([tag, value]) => {
...
async function processFileFn(file, data, results, picker) {
	...
	const colorVariants = results.files
		.filter((variantFile) => {
...
function addTagRegionFn(picker, title, settingKey) {
	...
	const tagsParent = $(
		`<div class="form-group favorites kinemancer"><label>${title}</label><div class="form-fields paths tags"></div></div>`
	);
