...
export class StatefulVideo {
	...
	static determineCurrentDelegator() {
		...
		delegateDebounce = foundry.utils.debounce(async () => {
			...
			if (
				newDelegator !== currentDelegator &&
				!newDelegator.isGM &&
				newDelegator === game.user &&
				!lib.isGMConnected()
			) {
				// Grab all stateful video's current state
				let updates = {};
				StatefulVideo.getAll().forEach((statefulVideo) => {
					updates[CONSTANTS.DELEGATED_STATEFUL_VIDEOS_FLAG + "." + statefulVideo.delegationUuid] =
						statefulVideo.flags.getData();
				});
	...
	static registerHooks() {
		...
		Hooks.on("updateUser", (user, data) => {
			...
			Object.keys(statefulVideos).forEach((key) => {
				...
				StatefulVideo.onUpdate(
					statefulVideo.document, // Construct a similar diff as normal video updates would create
		...
		Hooks.on("refreshTile", (placeableObject) => {
			if (
				!placeableObject.isVideo ||
				!foundry.utils.getProperty(placeableObject.document, CONSTANTS.STATES_FLAG)?.length
			)
				return;
	...
	static makeStateSelect(statefulVideo) {
		...
		for (const [index, state] of statefulVideo.flags.states.entries()) {
			select.append(
				`<option ${index === statefulVideo.flags.currentStateIndex ? "selected" : ""} value="${index}">${state.name}</option>`
			);
		}
		return select;
	}
	...
	static async renderStatefulVideoHud(app, html) {
		...
		const selectColorButton =
			$(`<div class="ats-hud-control-icon ats-stateful-video-ui-button" data-tooltip-direction="UP" data-tooltip="Change Color & Variant">
      ...
      ${statefulVideoColor.color ? `<div class="ats-color-button" style="${statefulVideoColor.color}"></div>` : ""}
		...
		if (statefulVideo) {
			...
			for (const [index, state] of statefulVideo.flags.states.entries()) {
				...
				const stateBtn = StatefulVideo.makeHudButton(
					state.name,
					state.icon,
					currentState ? "background-color: #048d6e;" : ""
				);
		...
		const fileSearchQuery = lib.getCleanWebmPath(placeableDocument).replace(".webm", "*.webm");

		const cleanTexturePath = decodeURIComponent(placeableDocument.texture.src);

		const baseVariation = cleanTexturePath.includes("_(") ? cleanTexturePath.split("_(")[1].split(")")[0] : "base";

		const internalVariation = cleanTexturePath.includes("_[") ? cleanTexturePath.split("_[")[1].split("]")[0] : "base";

		const colorVariation = cleanTexturePath.includes("__") ? cleanTexturePath.split("__")[1].split(".")[0] : "base";

		await lib
			.getWildCardFiles(fileSearchQuery)
			.then((results) => {
				return results.map(decodeURIComponent);
			})
			.then((results) => {
				const nonThumbnails = results
					.filter((filePath) => !filePath.includes("_thumb"))
					.sort((a, b) => {
				...
				const internalVariations = Object.values(
					nonThumbnails.reduce((acc, filePath) => {
						const specificInternalVariation = filePath.includes("_[")
							? filePath.split("_[")[1].split("]")[0].replace("_", " ").trim()
							: "base";
						const specificBaseVariation = filePath.includes("_(") ? filePath.split("_(")[1].split(")")[0] : "base";
						const specificColorVariation = filePath.includes("__") ? filePath.split("__")[1].split(".")[0] : "base";
				...
				if (statefulVideo) {
					statefulVideo.allInternalVariations = nonThumbnails.reduce((acc, filePath) => {
						...
						const specificInternalVariation = filePath.includes("_[") ? filePath.split("_[")[1].split("]")[0] : false;
					...
					statefulVideo.allColorVariations = nonThumbnails.reduce((acc, filePath) => {
						...
						const colorSpecificBaseVariation = filePath.includes("_(") ? filePath.split("_(")[1].split(")")[0] : "base";
				...
				const colorVariations = Object.values(
					nonThumbnails.reduce((acc, filePath) => {
						const colorSpecificBaseVariation = filePath.includes("_(") ? filePath.split("_(")[1].split(")")[0] : "base";
				...
				if (internalVariations.length > 1) {
					const variationContainer = $(
						`<div class="ats-variations ${colorVariations.length > 1 ? "ats-bottom-white-divider" : ""}"></div>`
					);
					...
					for (const variation of internalVariations) {
						...
						button.on("pointerdown", async () => {
							...
							const hud = placeable instanceof getTokenClass() ? canvas.tokens.hud : canvas.tiles.hud;
							placeable.control();
							hud.bind(placeable);
						});
					}
				...
				if (colorVariations.length > 1) {
					const colorContainer = $(
						`<div class="ats-color-container ${internalVariations.length > 1 ? "ats-top-divider" : ""}"></div>`
					);
	...
	preloadAssets() {
		...
		const textures = this.flags.states.map((state) => {
			return state.file ? this.flags.folderPath + "/" + state.file : this.flags.baseFile;
		});
	...
	updateSelect() {
		...
		for (const [index, state] of this.flags.states.entries()) {
			this.select.append(
				`<option ${index === this.flags.currentStateIndex ? "selected" : ""} value="${index}">${state.name}</option>`
			);
		}
		this.updateHudScale();
	}
	...
	static onPreUpdate(placeableDoc, changes) {
		...
		if (
			foundry.utils.hasProperty(diff, "texture.src") &&
			!foundry.utils.hasProperty(diff, CONSTANTS.CURRENT_STATE_FLAG) &&
			statefulVideo
		) {
	...
	static onUpdate(placeableDoc, changes, firstUpdate = false) {
		...
		if (!statefulVideo) {
			if (!placeableDoc.object.isVideo || !foundry.utils.getProperty(placeableDoc, CONSTANTS.STATES_FLAG)?.length)
				return;
		...
		if (foundry.utils.hasProperty(changes, CONSTANTS.STATES_FLAG)) {
			...
			return placeableDoc.update({
				[CONSTANTS.CURRENT_STATE_FLAG]:
					statefulVideo.flags.currentState.behavior === CONSTANTS.BEHAVIORS.RANDOM_STATE
		...
		if (
			foundry.utils.hasProperty(changes, CONSTANTS.CURRENT_STATE_FLAG) ||
			firstUpdate ||
			statefulVideo.flags.previousState.behavior === CONSTANTS.BEHAVIORS.RANDOM_STATE
		) {
			statefulVideo.setupRandomTimers();
	...
	static isDataValid(flags, data) {
		const previousStateFlagDifferent =
			data?.[CONSTANTS.PREVIOUS_STATE_FLAG] !== undefined &&
			flags.data[CONSTANTS.FLAG_KEYS.PREVIOUS_STATE] !== data?.[CONSTANTS.PREVIOUS_STATE_FLAG];
		const currentStateFlagDifferent =
			data?.[CONSTANTS.CURRENT_STATE_FLAG] !== undefined &&
			flags.data[CONSTANTS.FLAG_KEYS.CURRENT_STATE] !== data?.[CONSTANTS.CURRENT_STATE_FLAG];
		const queuedStateFlagDifferent =
			data?.[CONSTANTS.QUEUED_STATE_FLAG] !== undefined &&
			flags.data[CONSTANTS.FLAG_KEYS.QUEUED_STATE] !== data?.[CONSTANTS.QUEUED_STATE_FLAG];
		const previousFlagIsRandomState =
			flags.data.states[data?.[CONSTANTS.PREVIOUS_STATE_FLAG] ?? flags.data[CONSTANTS.FLAG_KEYS.CURRENT_STATE]]
				?.behavior === CONSTANTS.BEHAVIORS.RANDOM_STATE;
		return (
			previousStateFlagDifferent || currentStateFlagDifferent || queuedStateFlagDifferent || previousFlagIsRandomState
		);
	}
	...
	async update(data) {
		...
		const deconstructedData = Object.fromEntries(
	...
	handleLoopBehavior(options, startTime, endTime = 0) {
		...
		if (
			startTime === 0 &&
			loopDuration === this.duration &&
			!this.flags.queuedStateIndexIsDifferent &&
			noRandomTimers
		) {
			return {
				loop: true,
	...
	handleOnceBehavior(options, startTime, endTime) {
		...
		if (
			this.flags.currentStateIsOnceThenStill &&
			Number(Date.now()) >= this.flags.data.updated + endTime &&
			!this.ignoreDate
		) {
			this.playing = false;
			this.still = true;
...
class Flags {
	...
	get currentStateIsStill() {
		return (
			this.currentState.behavior === CONSTANTS.BEHAVIORS.STILL ||
			this.currentState.behavior === CONSTANTS.BEHAVIORS.STILL_HIDDEN
		);
	}
	...
	get currentStateIsRandom() {
		return (
			this.currentState.behavior === CONSTANTS.BEHAVIORS.RANDOM ||
			this.currentState.behavior === CONSTANTS.BEHAVIORS.RANDOM_IF
		);
	}
	...
	getData() {
		...
		if (currentDelegator && !currentDelegator.isGM) {
			const userFlags = foundry.utils.getProperty(
				currentDelegator,
				CONSTANTS.DELEGATED_STATEFUL_VIDEOS_FLAG + "." + this.delegationUuid
			);
	...
	determineNextRandomStates(stateIndex = null) {
		...
		return this.states
			.filter((s) => {
				return (
					s.behavior === CONSTANTS.BEHAVIORS.RANDOM ||
					(s.behavior === CONSTANTS.BEHAVIORS.RANDOM_IF && s.randomState === state.id)
				);
			})
	...
	determineNextStateIndex(stateIndex = null) {
		...
		switch (state?.behavior) {
			...
			case CONSTANTS.BEHAVIORS.RANDOM_STATE: {
				const nextStates = state.randomState
					.map((id) => this.states.findIndex((s) => s.id === id))
					.filter((i) => i > -1);
	...
	determineFile(stateIndex, statefulVideo) {
		...
		if (this.useFiles && this.folderPath) {
			let filePath = state.file ? this.folderPath + "/" + state.file : this.baseFile;

			const baseVariation = filePath.includes("_(") ? filePath.split("_(")[1].split(")")[0] : "base";
