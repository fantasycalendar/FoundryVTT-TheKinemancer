...
export class StatefulVideo {
  ...
  static async renderStatefulVideoHud(app, html) {
    ...
    const baseFile = decodeURI(placeableDocument.texture.src).split("_(")[0].split("__")[0].replace(".webm", "") + "*.webm";
  ...
  handleOnceBehavior(startTime, endTime) {
    ...
    if (this.flags.currentStateIsOnceThenStill && Number(Date.now()) >= (this.flags.data.updated + this.duration)) {
      return {
        playing: true, loop: false, offset: this.duration / 1000
      };
    }
